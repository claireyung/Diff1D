  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%  DIFF1D_Jq - 1D Diffusion equation solver                          %
%              Turbulent heat flux plotting                          %
% This script plots the average turbulent heat flux from             %
% several Diff1D runs.                                               %
%                                                                    %
...
% Runs to plot:
out_folder = 'Kelvin/';
runsP1 = {'001','001_noTIW','002','002_noTIW'};
runsP2 = {'001_PP','001_noTIW_PP','002_PP','002_noTIW_PP'};

names = {'Up Kelvin','Up Kelvin no TIW', ...
         'Down Kelvin', 'Down Kelvin no TIW'};
titles = {'KPP Mixing', 'P\&P Mixing'}

% Time limits to average over (days):
t1 = 0;
t2 = 30;
...
dots = 720/4;
linesize = [20 20 7 20];
colors = {'k',[0 0.4471    0.7412],[0.4941    0.1843    0.5569], 'r'};

% --------------------------------------------------------------------

runs = [runsP1, runsP2];
nrunsP1 = length(runsP1);
nrunsP2 = length(runsP2);
nruns = nrunsP1+nrunsP2;

%Setup time limits:
[tmp tII] = min(abs(t/86400-t1));
[tmp tFF] = min(abs(t/86400-t2));
tvec = tII:(tFF-1);
...
%First panel:
Jqplot1 = subplot('Position',[0.2512    0.3423    0.1854 ...
                    0.5495]);
title(titles{1});
...
for i=1:nrunsP1
plot(Jq{i},z,'-','LineWidth',3,'color',colors{i});
end
...
for i=(nrunsP1+1):nruns
plot(Jq{i},z,'-','LineWidth',3,'color',colors{i-nrunsP1});
end
